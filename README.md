...
### windowsの場合
- いくつか選択肢がありますが、インストール難易度の低い順に記載しています。どうしてもwindowsちゃんがいいんじゃい・・・という方は、下記のいずれかでご対応お願いいたします。なお開催時間の関係で、GUIクライアントは今回サポート致しません。ご了承ください。

1. sourcetreeなどのGUIクライアント（gitflow込みでインストールは簡単。ハンズオンでは操作や設定のサポートは致しません）
2. cygwin上でgit/gitflow（cygwin導入済みであればこちらがおすすめ。gitflowのインストールがなかなかメンドクサイ）
3. git for windows（git for windows自体のインストールは簡単ですが、gitflowのが最悪にメンドクサイ）
...
- GUIクライアントを社内環境から利用する場合、プロキシの設定が倒れるほどめんどくさいので心が折れるかもしれません。改めてこの場でLinux/macの使用をお勧めいたします。なおプロキシの必要ない環境であれば、特に問題なくすっきり使えます。
