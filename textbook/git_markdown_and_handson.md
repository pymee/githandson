...
- himitsu.txtは秘密なので、githubにプッシュするのはもちろん、gitの配下には置きたくありません。
- そういう場合、**.gitignore**を使用して、gitの管理下から外すことができます。見てみましょう。
```
cat ~/githandson/.gitgnore
```
- パスワードなどを記載したファイルや、個人の開発環境の設定などをgithubにプッシュしてしまわないように使うことが多いです。
...
```
git flow release start 18.07.1
```
### 解説
- git flowでは、developからいきなりリリースするのではなく、一旦releaseブランチを作成し、そこにdevelopのコピーを作成します。リリースに必要なファイル群をまとめたり、顧客に見せないファイルを避けたりなどの作業を行います。
- バージョン番号「18.07.1」は、この作業における7月の最初のリリースであることを意味します。 ver1.0などよりわかりやすいため、dockerやfirefoxなどで採用されている方法です。
---
リリース
==
- リリース準備が終わったら、いよいよリリースします。
```
git flow release finish
```
- エディタが起動します。コミットメッセージの編集をする場合は編集して保存します。
- もう一度エディタが起動します。ここでバージョン番号を記入します。普通は、release startしたときの番号を記入します。
- ローカルでのリリースが完了しました。最後の仕上げに、ローカルのリリースをgithubにプッシュします。
```
git push 
```
